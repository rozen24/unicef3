...
# Update Module 19 Lesson 3 with modern UI - Line-based replacement

# Read the file
with open('js/data.js', 'r', encoding='utf-8') as f:
    lines = f.readlines()

# Find the start and end of Lesson 3
start_idx = None
end_idx = None

for i, line in enumerate(lines):
    if 'id: "ch19-lesson-3"' in line:
        # Walk back to find the opening {
        for j in range(i, max(0, i-5), -1):
            if lines[j].strip() == '{':
                start_idx = j  # This is the opening { of lesson 3
                break
    elif start_idx is not None and 'id: "ch19-lesson-4"' in line:
        # Walk back to find the closing }, of lesson 3
        for j in range(i, max(0, i-5), -1):
            if lines[j].strip() == '{':
                end_idx = j - 1  # The line before the { is },
                break
        break
...
print(f"Found Lesson 3: lines {start_idx+1} to {end_idx+1} (total {end_idx-start_idx+1} lines)")
...
# Replace lines
new_lines = lines[:start_idx] + new_lesson_lines + lines[end_idx+1:]

# Write back
with open('js/data.js', 'w', encoding='utf-8') as f:
    f.writelines(new_lines)

print(f"✓ Module 19 Lesson 3 successfully redesigned!")
print(f"   Replaced {end_idx-start_idx+1} lines with {len(new_lesson_lines)} lines")
